...
export function buildSubsystemEnforcementDeclarations(): SubsystemEnforcementDeclaration[] {
  return [
    ...
    d({
      subsystemId: "prose_narrative_realism_cluster5",
      subsystemName: "Prose & narrative realism (Cluster 5 shaping + validation)",
      enforcementClass: "advisory_runtime",
      authorityClass: "advisory_runtime",
      participatesInCanonicalRuntime: true,
      affectsCanonicalOutput: true,
      ...
      canonicalOutputEffectClass: "soft_constraint",
      semanticTruthNotes:
        "Deterministic prose-realism prompt lines and post-gen validation on canonical scene generation; observational cockpit panel.",
      validationFlags: ["cluster5_prose_realism", "canonical_scene_generation_path"],
    }),
    d({
      subsystemId: "character_simulation_runtime_cluster8",
      subsystemName: "Character simulation runtime (Cluster 8 mind/voice/relationship emergence)",
      ...
      canonicalOutputEffectClass: "soft_constraint",
      semanticTruthNotes:
        "Canonical scene generation: CLUSTER8 prompt lines + hash when active; emergence plan attached to governance bundle; validation is advisory (warnings).",
      validationFlags: ["cluster8_character_simulation", "canonical_scene_generation_path"],
    }),
    d({
      subsystemId: "ricre_research_canon_reconciliation",
      subsystemName: "RICRE research ingestion & canon reconciliation (author-gated)",
      enforcementClass: "soft_enforced_runtime",
      authorityClass: "canonical_production",
      participatesInCanonicalRuntime: true,
      affectsCanonicalOutput: true,
      ...
      canonicalOutputEffectClass: "soft_constraint",
      semanticTruthNotes:
        "Author-triggered research is ingested with provenance; raw web text never silently becomes story canon. Accepted rows flow into RICRE_ACCEPTED_CANON prompt lines only after explicit author reconciliation.",
      validationFlags: ["ricre_research_ingestion", "canonical_scene_generation_path"],
    }),
    d({
      subsystemId: "human_gravity_runtime_cluster6",
      subsystemName: "Human gravity runtime (Cluster 6 attachment/stakes/consequence/burden)",
      enforcementClass: "soft_enforced_runtime",
      authorityClass: "canonical_production",
      participatesInCanonicalRuntime: true,
      affectsCanonicalOutput: true,
      ...
      canonicalOutputEffectClass: "soft_constraint",
      semanticTruthNotes:
        "Canonical scene generation: CLUSTER6 prompt lines + hash when active; prose-realism seed may use humanGravityScore; no-reset rule can block generationText save when upstream pressure is unaccounted in prose.",
      validationFlags: ["cluster6_human_gravity_runtime", "canonical_scene_generation_path", "no_reset_validity_gate"],
    }),
    d({
