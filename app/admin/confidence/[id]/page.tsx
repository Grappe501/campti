...
import { updateConfidenceProfile } from "@/app/actions/ontology";
import { RECORD_TYPE_ORDER, VISIBILITY_ORDER } from "@/lib/constitutional-rule-constants";
import { fieldClass, labelClass, labelSpanClass } from "@/lib/admin-styles";
import { PageHeader } from "@/components/page-header";
import { getConfidenceProfileByIdForAdmin } from "@/lib/data-access";
...
export default async function EditConfidenceProfilePage({ params, searchParams }: Props) {
  const { id } = await params;
  const sp = await searchParams;
  const row = await getConfidenceProfileByIdForAdmin(id);
  if (!row) notFound();
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-8">
      ...
      <form action={updateConfidenceProfile} className="space-y-4 rounded-xl border border-stone-200 bg-white p-6 shadow-sm">
