...
import { createConfidenceProfile } from "@/app/actions/ontology";
...
export default async function NewConfidenceProfilePage({ searchParams }: Props) {
  const sp = await searchParams;

  return (
    <div className="mx-auto max-w-2xl space-y-8">
      <Link href="/admin/confidence" className="text-sm text-amber-900 hover:underline">
        ← Confidence
      </Link>
      <PageHeader title="New confidence profile" description="Numeric rank orders profiles (lower = weaker evidence)." />
      <AdminFormError error={sp.error} />

      <form action={createConfidenceProfile} className="space-y-4 rounded-xl border border-stone-200 bg-white p-6 shadow-sm">
        ...
        <label className={labelClass}>
          <span className={labelSpanClass}>Numeric value</span>
          <input name="numericValue" type="number" required min={0} max={100} defaultValue={3} className={fieldClass} />
        </label>
