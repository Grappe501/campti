import Link from "next/link";
import { createOpenQuestion } from "@/app/actions/questions";
import { AdminFormError } from "@/components/admin-form-error";
import { AdminTable } from "@/components/admin-table";
import { EmptyState } from "@/components/empty-state";
import { PageHeader } from "@/components/page-header";
import { StatusBadge } from "@/components/status-badge";
import {
  getEvents,
  getOpenQuestions,
  getPeople,
  getPlaces,
  getSources,
} from "@/lib/data-access";
import { fieldClass, labelClass, labelSpanClass } from "@/lib/admin-styles";

export const dynamic = "force-dynamic";

type Search = { status?: string; priority?: string; error?: string };

export default async function AdminQuestionsPage({
  searchParams,
}: {
  searchParams: Promise<Search>;
}) {
  const sp = await searchParams;
  const priorityNum =
    sp.priority && /^\d+$/.test(sp.priority)
      ? Math.min(5, Math.max(1, Number(sp.priority)))
      : undefined;
  const [questions, people, places, events, sources] = await Promise.all([
    getOpenQuestions({
      status: sp.status,
      priority: priorityNum,
    }),
    getPeople(),
    getPlaces(),
    getEvents(),
    getSources(),
  ]);

  const filterHref = (next: Partial<Search>) => {
    const merged = { ...sp, ...next };
    const p = new URLSearchParams();
    if (merged.status) p.set("status", merged.status);
    if (merged.priority) p.set("priority", merged.priority);
    const q = p.toString();
    return q ? `/admin/questions?${q}` : "/admin/questions";
  };

  const statuses = ["open", "researching", "resolved"];

  return (
    <div className="mx-auto max-w-5xl space-y-10">
      <PageHeader
        title="Open questions"
        description="Contradictions, missing records, oral history gaps, and research threads. This is a primary author workspace."
      />
      <AdminFormError error={sp.error} />

      <section className="rounded-xl border border-amber-200/80 bg-amber-50/40 p-6 shadow-sm">
        <h2 className="text-lg font-medium text-stone-900">Record a question</h2>
        <form action={createOpenQuestion} className="mt-4 space-y-4">
          <label className={labelClass}>
            <span className={labelSpanClass}>Title</span>
            <input name="title" required className={fieldClass} />
          </label>
          <label className={labelClass}>
            <span className={labelSpanClass}>Description</span>
            <textarea name="description" rows={4} className={fieldClass} />
          </label>
          <div className="grid gap-4 sm:grid-cols-2">
            <label className={labelClass}>
              <span className={labelSpanClass}>Status</span>
              <select name="status" className={fieldClass} defaultValue="open">
                {statuses.map((s) => (
                  <option key={s} value={s}>
                    {s}
                  </option>
                ))}
              </select>
            </label>
            <label className={labelClass}>
              <span className={labelSpanClass}>Priority (1–5, optional)</span>
              <input name="priority" type="number" min={1} max={5} className={fieldClass} />
            </label>
          </div>
          <div className="grid gap-4 sm:grid-cols-2">
            <label className={labelClass}>
              <span className={labelSpanClass}>Linked person</span>
              <select name="linkedPersonId" className={fieldClass}>
                <option value="">—</option>
                {people.map((p) => (
                  <option key={p.id} value={p.id}>
                    {p.name}
                  </option>
                ))}
              </select>
            </label>
            <label className={labelClass}>
              <span className={labelSpanClass}>Linked place</span>
              <select name="linkedPlaceId" className={fieldClass}>
                <option value="">—</option>
                {places.map((pl) => (
                  <option key={pl.id} value={pl.id}>
                    {pl.name}
                  </option>
                ))}
              </select>
            </label>
          </div>
          <div className="grid gap-4 sm:grid-cols-2">
            <label className={labelClass}>
              <span className={labelSpanClass}>Linked event</span>
              <select name="linkedEventId" className={fieldClass}>
                <option value="">—</option>
                {events.map((e) => (
                  <option key={e.id} value={e.id}>
                    {e.title}
                  </option>
                ))}
              </select>
            </label>
            <label className={labelClass}>
              <span className={labelSpanClass}>Linked source</span>
              <select name="linkedSourceId" className={fieldClass}>
                <option value="">—</option>
                {sources.map((s) => (
                  <option key={s.id} value={s.id}>
                    {s.title}
                  </option>
                ))}
              </select>
            </label>
          </div>
          <button type="submit" className="rounded-full bg-stone-900 px-5 py-2 text-sm font-medium text-amber-50 hover:bg-stone-800">
            Save question
          </button>
        </form>
      </section>

      <section className="space-y-4">
        <h2 className="text-lg font-medium text-stone-900">All open questions</h2>
        <div className="flex flex-wrap gap-2 text-sm">
          <span className="text-xs font-semibold uppercase text-stone-500">Status</span>
          {statuses.map((s) => (
            <Link
              key={s}
              href={filterHref({ status: s })}
              className={`rounded-full px-2 py-0.5 capitalize ${sp.status === s ? "bg-stone-900 text-amber-50" : "border border-stone-200 bg-white"}`}
            >
              {s}
            </Link>
          ))}
          <Link href={filterHref({ status: undefined })} className="text-amber-900 hover:underline">
            Any
          </Link>
        </div>
        <div className="flex flex-wrap items-center gap-2 text-sm">
          <span className="text-xs font-semibold uppercase text-stone-500">Priority</span>
          {[1, 2, 3, 4, 5].map((n) => (
            <Link
              key={n}
              href={filterHref({ priority: String(n) })}
              className={`rounded-full px-2 py-0.5 ${sp.priority === String(n) ? "bg-stone-900 text-amber-50" : "border border-stone-200 bg-white"}`}
            >
              {n}
            </Link>
          ))}
          <Link href={filterHref({ priority: undefined })} className="text-amber-900 hover:underline">
            Any
          </Link>
        </div>

        {questions.length === 0 ? (
          <EmptyState title="No questions match." />
        ) : (
          <AdminTable
            rows={questions}
            rowKey={(q) => q.id}
            columns={[
              {
                key: "title",
                header: "Question",
                cell: (q) => (
                  <Link href={`/admin/questions/${q.id}`} className="font-medium text-amber-900 hover:underline">
                    {q.title}
                  </Link>
                ),
              },
              {
                key: "status",
                header: "Status",
                cell: (q) => <StatusBadge label={q.status} />,
              },
              {
                key: "pri",
                header: "P",
                className: "w-8 tabular-nums",
                cell: (q) => q.priority ?? "—",
              },
            ]}
          />
        )}
      </section>
    </div>
  );
}
